...
namespace winrt::HV4DTBASIC::implementation
{
	...
	HV4DCLASST::HV4DCLASST(HV4D::IHV4DCOBJECT const& sender)
	{
		HV4DListSupportedInterfaces();
	...
	HV4DCLASST::HV4DCLASST(HV4D::IHV4DCOBJECT const& sender, winrt::guid const& boxed, WF::IInspectable const& args)
	{
		HV4DListSupportedInterfaces();
	...
	std::uint64_t HV4DCLASST::HV4DCreatedDateTime()
	{
	...
	std::uint64_t HV4DCLASST::HV4DUpdatedDateTime()
	{
	...
	HV4D::IHV4DCOBJECT HV4DCLASST::HV4DSender()
	{
		HV4DSetUpdatedDateTime();
	...
	void HV4DCLASST::HV4DSender(HV4D::IHV4DCOBJECT const& e)
	{
		HV4DSetUpdatedDateTime();
	...
	std::uint64_t HV4DCLASST::HV4DTTypeID()
	{
		HV4DSetUpdatedDateTime();
	...
	HV4D::IHV4DRETURN HV4DCLASST::HV4DCLASSTFromHstring(winrt::hstring const& ns, winrt::hstring const& c)
	{
		try
		{
			tagCLASST = std::tuple{ ns.data(), c.data() };
	...
	HV4D::IHV4DRETURN HV4DCLASST::HV4DCLASSTFromMAXPATH(TBASIC::HV4DMAXPATH const& ns, TBASIC::HV4DMAXPATH const& c)
	{
		...
		try
		{
			tagCLASST = std::tuple{ nmspace.data(), coclass.data() };
	...
	{
		std::tuple<std::wstring, std::wstring> abs = tagCLASST;
	...
	HV4D::IHV4DRETURN HV4DCLASST::HV4DCLASSTToHV4DMAXPATH(TBASIC::HV4DMAXPATH& ns, TBASIC::HV4DMAXPATH& c)
	{
		...
		ns.HV4DMAXPATHFromHstring(std::get<0>(abs));

		c.HV4DMAXPATHFromHstring(std::get<1>(abs));
	...
	HV4D::IHV4DRETURN HV4DCLASST::HV4DCLASSTToProj(TBASIC::HV4DCLASST& e)
	{
		...
		e.HV4DCLASSTFromHstring(std::get<0>(abs), std::get<1>(abs));
	...
	HV4D::IHV4DRETURN HV4DCLASST::HV4DCLASSTIsEqualHstring(winrt::hstring const& ns, winrt::hstring const& c)
	{
		...
		if (std::get<0>(abs) == ns && std::get<1>(abs) == c)
		{
	...
	HV4D::IHV4DRETURN HV4DCLASST::HV4DCLASSTIsEqualHV4DMAXPATH(TBASIC::HV4DMAXPATH const& ns, TBASIC::HV4DMAXPATH const& c)
	{
		...
		if (std::get<0>(abs) == nmspace && std::get<1>(abs) == coclass)
		{
			return HV4D::HV4D_TRUE{};

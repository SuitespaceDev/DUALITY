...
namespace winrt::Duality137::implementation
{
	...
	void Machine::InitializeIndex()
	{
		Pages = new std::unordered_map<CTBASIC::CHV4DKEY, std::function<bool()>, CTBASIC::CHV4DKEYFN>{
			...
			{ {winrt::guid{0x9126C512, 0x05D3, 0x4CFE, {0xBF, 0xC0, 0x6E, 0xBC, 0x18, 0x97, 0xDE, 0x0F}}   ,L"DigitalTwinPage"          }, std::function<bool()>{ [this]() { return MachineFrame().Navigate(winrt::xaml_typename<Duality137::DigitalTwinPage>()); } } },
			...
			{ {winrt::guid{0x5E8FCF0C, 0x62AD, 0x46FA, {0xBD, 0xFE, 0xA9, 0x4B, 0xFB, 0x18, 0xD5, 0xC9}}   ,L"BindBinaryPage"          }, std::function<bool()>{ [this]() { return MachineFrame().Navigate(winrt::xaml_typename<Duality137::BindBinaryPage>()); } } },
			{ {winrt::guid{0x6515BE6A, 0x7349, 0x4DBC, {0x80, 0x3E, 0x8C, 0x19, 0x0B, 0x19, 0x87, 0x58}}   ,L"BindJSONPage"            }, std::function<bool()>{ [this]() { return MachineFrame().Navigate(winrt::xaml_typename<Duality137::BindJSONPage>()); } } },
			{ {winrt::guid{0x3D0ED970, 0x7D01, 0x40D8, {0xA6, 0x79, 0x72, 0xD5, 0x6E, 0x41, 0x2E, 0xBB}}   ,L"BindXMLPage"             }, std::function<bool()>{ [this]() { return MachineFrame().Navigate(winrt::xaml_typename<Duality137::BindXMLPage>()); } } },
			{ {winrt::guid{0xDC7DA50F, 0x8D48, 0x4EEC, {0x93, 0xF5, 0xF1, 0x2D, 0x4A, 0xEA, 0x28, 0x18}}   ,L"BindXAMLPage"            }, std::function<bool()>{ [this]() { return MachineFrame().Navigate(winrt::xaml_typename<Duality137::BindXAMLPage>()); } } },
			{ {winrt::guid{0x2618883D, 0x37BA, 0x460B, {0xB3, 0xEF, 0x6D, 0x61, 0x77, 0x01, 0xCB, 0xA6}}   ,L"BindQIFPage"             }, std::function<bool()>{ [this]() { return MachineFrame().Navigate(winrt::xaml_typename<Duality137::BindQIFPage>()); } } },
			{ {winrt::guid{0xEDC1BCCE, 0xD74E, 0x4C1C, {0x9D, 0xD7, 0x7C, 0x5E, 0xBD, 0xAF, 0x31, 0xB0}}   ,L"BindPostgreSQLPage"      }, std::function<bool()>{ [this]() { return MachineFrame().Navigate(winrt::xaml_typename<Duality137::BindPostgreSQLPage>()); } } },
			...
			{ {winrt::guid{0x1FB6C794, 0xC781, 0x436A, {0x8B, 0xD6, 0xC7, 0xE3, 0x04, 0x9C, 0x44, 0x91}}   ,L"PeripheralDevicePage"		}, std::function<bool()>{ [this]() { return MachineFrame().Navigate(winrt::xaml_typename<Duality137::PeripheralDevicePage>()); } } },
			{ {winrt::guid{0xAFB7FB2D, 0x4C2D, 0x4A47, {0x85, 0x0A, 0x05, 0xCE, 0x3F, 0xB1, 0xCA, 0x1B}}   ,L"StorageDevicePage"        }, std::function<bool()>{ [this]() { return MachineFrame().Navigate(winrt::xaml_typename<Duality137::StorageDevicePage>()); } } },
			{ {winrt::guid{0x19E262AB, 0xC56E, 0x47A7, {0xAA, 0xE9, 0xA9, 0x45, 0x9F, 0x47, 0x26, 0xBF}}   ,L"RandomMemoryPage"         }, std::function<bool()>{ [this]() { return MachineFrame().Navigate(winrt::xaml_typename<Duality137::RandomMemoryPage>()); } } },
			{ {winrt::guid{0x36E17529, 0x39A7, 0x4AEC, {0x9E, 0xF9, 0x1C, 0xAF, 0x31, 0x88, 0xB5, 0x6A}}   ,L"ProcessorCorePage"        }, std::function<bool()>{ [this]() { return MachineFrame().Navigate(winrt::xaml_typename<Duality137::ProcessingCorePage>()); } } },
			...
			{ {winrt::guid{0x0DB7C610, 0x447E, 0x423F, {0x85, 0xC3, 0x44, 0xE8, 0x1D, 0x7F, 0x16, 0xB3}}   ,L"ResourcePage"             }, std::function<bool()>{ [this]() { return MachineFrame().Navigate(winrt::xaml_typename<Duality137::ResourcePage>()); } } },
			{ {winrt::guid{0xECFE799D, 0x62A4, 0x498A, {0xA3, 0x98, 0x20, 0xE6, 0xE4, 0xAD, 0x2A, 0x9C}}   ,L"UserControlPage"          }, std::function<bool()>{ [this]() { return MachineFrame().Navigate(winrt::xaml_typename<Duality137::UserControlPage>()); } } },
			...
			{ {winrt::guid{0x71E2E256, 0x72DF, 0x4403, {0x9D, 0xDA, 0x09, 0xBB, 0xC1, 0x1A, 0xBD, 0x7D}}   ,L"ToolBarPage"              }, std::function<bool()>{ [this]() { return MachineFrame().Navigate(winrt::xaml_typename<Duality137::ToolBarPage>()); } } },
			{ {winrt::guid{0x69840795, 0x22B9, 0x4E5D, {0xA3, 0x36, 0x8A, 0x65, 0x2E, 0x83, 0x68, 0x3E}}   ,L"WidgetPage"               }, std::function<bool()>{ [this]() { return MachineFrame().Navigate(winrt::xaml_typename<Duality137::WidgetPage>()); } } },
			{ {winrt::guid{0x8304CF6F, 0x5ACD, 0x4B68, {0xA5, 0x6B, 0x5F, 0xF1, 0x89, 0x28, 0x73, 0x74}}   ,L"ContextMenuPage"          }, std::function<bool()>{ [this]() { return MachineFrame().Navigate(winrt::xaml_typename<Duality137::ContextMenuPage>()); } } },
